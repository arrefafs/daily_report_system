...
public class FrontController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
                throws ServletException, IOException  {
                    ...
                    //サーブレットコンテキスト,リクエスト,レスポンスをActionインスタンスのフィ―ルドに設定
    ...
    try {
        ...
        //該当するActionオブジェクトを作成(例:actions.EmployeeActionオブジェクトactions.ActionBaseオブジェクト
        action = (ActionBase)(type.asSubclass(ActionBase.class))
            ...
            | IllegalArgumentException | InvocationTargetException| NoSuchMethodException e){
                ...
                //リクエストパラメータに認定されている"action"の値が不正の場合
                //エラー処理を行うActionオブジェクトを作成

                action = new UnknowAction();
            }
               return action;
    }
